...
import {
  ...
  Search, Filter, Phone, Sprout, ClipboardList, Ruler, ShieldAlert
} from 'lucide-react';
...
export default function MobileAgroApp() {
  ...
  const [reportData, setReportData] = useState({
    field_id: '', type: 'кўрик', note: '', image: null, lat: null, lon: null, accuracy: null
  });
  ...
  useEffect(() => {
    if (!agroProfile) return;
    const channel = supabase
      .channel('global_settings_changes')
      .on('postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'global_settings', filter: "key=eq.is_raid_active" },
        (payload) => { setIsRaidActive(payload.new.value); }
      )
      .subscribe();

    const checkInitialRaid = async () => {
      const { data } = await supabase.from('global_settings').select('value').eq('key', 'is_raid_active').single();
      if (data) setIsRaidActive(data.value);
    };
    checkInitialRaid();
  ...
  useEffect(() => {
    if (isRaidActive && agroProfile) {
      const id = navigator.geolocation.watchPosition(
        async (pos) => {
          await supabase.from('agronomists').update({
            last_lat: pos.coords.latitude,
            last_lng: pos.coords.longitude,
            last_seen: new Date().toISOString()
          }).eq('id', agroProfile.id);
        },
        (err) => console.error("Raid GPS Error:", err),
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      *::-webkit-scrollbar { display: none !important; width: 0 !important; height: 0 !important; }
      html, body { overscroll-behavior: none !important; position: fixed; width: 100%; height: 100%; overflow: hidden; background-color: #05110a; margin: 0; padding: 0; }
      #root { height: 100%; width: 100%; overflow-y: auto; overflow-x: hidden; -webkit-overflow-scrolling: touch; }
      ...
      @keyframes pulseRaid { 0% { opacity: 1; } 50% { opacity: 0.5; } 100% { opacity: 1; } }
      .raid-active { animation: pulseRaid 2s infinite; color: #ef4444; font-weight: bold; display: flex; align-items: center; gap: 5px; font-size: 0.7rem; }
    `;
  ...
  async function fetchAppData(agroId) {
    const { data: allTasks } = await supabase.from('tasks').select('*, fields(*)').eq('agronomist_id', agroId);
    setMyTasks(allTasks || []);
    ...
    const { data: fData } = await supabase.from('fields').select('*, farmers(*)').eq('agronomist_id', agroId);
    setMyFields(fData || []);
  ...
  async function fetchFieldHistory(fieldId) {
    ...
    const { data, error } = await supabase.from('field_operations').select('*').eq('field_id', fieldId).order('operation_date', { ascending: false });
  ...
  // --- ЯНГИ: РАСМНИ STORAGE ГА ЮКЛАШ ФУНКЦИЯСИ ---
  ...
  // --- ЯНГИЛАНГАН: ҲИСОБОТНИ РАСМ БИЛАН ЮБОРИШ ---
  const submitReport = async () => {
    ...
    try {
      let imageUrl = null;
      if (reportData.image) {
        imageUrl = await uploadPhoto(reportData.image);
      }
      ...
      const { error } = await supabase.from('field_operations').insert([{
        ...
        image_url: imageUrl, // Устун номи базада image_url бўлиши керак
        operation_date: new Date().toISOString()
      }]);

      if (!error) {
        if (selectedTaskId) await supabase.from('tasks').update({ status: 'completed' }).eq('id', selectedTaskId);
        alert("🚀 Ҳисобот ва расм муваффақиятли юборилди!");
        setShowReportModal(false);
        setSelectedTaskId(null);
        setReportData({ field_id: '', type: 'кўрик', note: '', image: null, lat: null, lon: null, accuracy: null });
        fetchAppData(agroProfile.id);
      } else {
        alert("Хато: " + error.message);
      }
    } catch (err) {
      alert("Расм юклашда хатолик: " + err.message);
    } finally {
      setAuthLoading(false);
    }
  };
  ...
  const openNavigation = (type) => {
    ...
    let url = '';
    if (type === 'google') url = `https://www.google.com/maps/search/?api=1&query=${lat},${lon}`;
            ...
            <div style={{ paddingTop: '10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                <div>
                  ...
                  {isRaidActive ? (
                    <div className="raid-active"><ShieldAlert size={14} />РЕЙД ФАОЛ</div>
                  ) : (
            ...
            <div style={{ paddingTop: '10px' }}>
              ...
              <div style={{ marginBottom: '25px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ ...inputWrapper, marginBottom: 0 }}>
                  ...
                  <input type="text" placeholder="Дала ёки фермер исми..." style={{ ...loginInput, borderRadius: '16px', background: 'rgba(255,255,255,0.1)', width: '100%', boxSizing: 'border-box', paddingRight: '15px' }} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                </div>
                ...
                {filteredFields.map(f => (
                  <div key={f.id} className="field-card" onClick={() => { setShowFieldPassport(f); fetchFieldHistory(f.id); }} style={{ ...THEME.glass, padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: '0.2s' }}>
                    <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                      <div style={{ width: '48px', height: '48px', borderRadius: '14px', background: 'rgba(16,185,129,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Sprout size={24} color={THEME.accent} /></div>
                      <div>
          ...
          <div style={styles.modalOverlay} onClick={() => setShowFieldPassport(null)}>
            <div className="bottom-sheet" style={bottomSheetContainer} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ marginTop: '25px', marginBottom: '25px' }}>
                <h4 style={{ fontSize: '1.1rem', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '800' }}><Clock size={18} color={THEME.accent} /> Амалиётлар тарихи</h4>
                {historyLoading ? (<div style={{ textAlign: 'center', padding: '20px' }}><Loader2 className="animate-spin" color={THEME.accent} /></div>) : fieldHistory.length === 0 ? (<div style={{ color: THEME.textSecondary, fontSize: '0.85rem', textAlign: 'center', padding: '15px', background: 'rgba(255,255,255,0.03)', borderRadius: '15px' }}>Тарих топилмади</div>) : (
                    ...
                    {fieldHistory.map((op) => (
                      <div key={op.id} style={{ position: 'relative', paddingLeft: '20px', paddingBottom: '20px' }}>
                        ...
                        <div style={{ background: 'rgba(255,255,255,0.04)', padding: '12px', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.06)' }}>
                          <div style={{ fontWeight: '700', fontSize: '0.9rem', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '6px' }}>{op.operation_type === 'inspection' ? <Search size={14} /> : op.operation_type === 'watering' ? <Droplets size={14} /> : <Bug size={14} />}{op.operation_type.toUpperCase()}</div>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                {showFieldPassport.location_center?.lat && (<button onClick={() => setNavMenu({ show: true, lat: showFieldPassport.location_center.lat, lon: showFieldPassport.location_center.lon })} style={secondaryBtn}><Navigation size={18} /> ХАРИТА</button>)}
                <button onClick={() => { const fId = showFieldPassport.id.toString(); setShowFieldPassport(null); handleFieldSelect(fId); setShowReportModal(true); }} style={{ ...loginBtn, flex: 2 }}>ҲИСОБОТ ҚЎШИШ</button>
              </div>
            </div>
          </div>
        )}
                  ...
                  <div style={{ marginTop: '15px' }}>
                    {fieldMissingGPS ? (
                      <div style={warningBox}>
                        ...
                        {!reportData.lat ? (
                          <button onClick={getGeoLocation} style={{ ...smallActionBtn, width: '100%', background: THEME.warning, color: '#000', fontWeight: 'bold' }}>{isLocating ? <Loader2 className="animate-spin" size={16} /> : <MapPin size={16} />} GPS НИ АНИҚЛАШ</button>
                        ) : (
                          <div>
                      ...
                      <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
                        ...
                        <div style={{ marginTop: '20px' }}>
                          ...
                          <div style={typeGrid}>
                            <TypeBtn active={reportData.type === 'Кўрик'} icon={<Map size={18} />} label="Кўрик" onClick={() => setReportData({ ...reportData, type: 'Кўрик' })} />
                            <TypeBtn active={reportData.type === 'Суғориш'} icon={<Droplets size={18} />} label="Суғориш" onClick={() => setReportData({ ...reportData, type: 'Суғориш' })} />
                            <TypeBtn active={reportData.type === 'Зараркунанда'} icon={<Bug size={18} />} label="Зараркунанда" onClick={() => setReportData({ ...reportData, type: 'Зараркунанда' })} />
                          </div>
                          <label style={{ ...styles.label, marginTop: '20px', display: 'block' }}>📸 Изоҳ ва Расм</label>
                          <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                            <label style={{
                                ...photoUploadCard, 
                                borderColor: reportData.image ? THEME.accent : 'rgba(255,255,255,0.2)',
                                background: reportData.image ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.05)'
                            }}>
                                ...
                                {reportData.image && <span style={{fontSize: '10px', color: THEME.accent, marginTop: '4px'}}>Танланди</span>}
                            </label>
                          ...
                          <button onClick={submitReport} style={loginBtn} disabled={authLoading}>
                             {authLoading ? <Loader2 className="animate-spin" /> : "ҲИСОБОТНИ ЮБОРИШ"}
                          </button>
                        </div>
                      </div>
                    )}
...
const PassportRow = ({ icon, label, value, isPhone }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(255,255,255,0.04)', padding: '12px', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.06)' }}>
    ...
    <div style={{ flex: 1 }}>
      ...
      {isPhone ? (<a href={`tel:${value}`} style={{ fontSize: '1rem', color: '#fff', textDecoration: 'none', fontWeight: '600' }}>{value}</a>) : (<div style={{ fontSize: '1rem', color: '#fff', fontWeight: '600' }}>{value}</div>)}
    </div>
  </div>
);
...
const bottomSheetContainer = { ...THEME.glass, width: '100%', maxWidth: '500px', borderBottomLeftRadius: 0, borderBottomRightRadius: 0, background: 'rgba(10, 20, 15, 0.95)', padding: '25px', paddingTop: '12px', maxHeight: '90vh', overflowY: 'auto' };
